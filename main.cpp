...
queue<code_task> task_queue;
mutex queue_mutex;
condition_variable pool_cv;
bool server_running = true;

unordered_map<int, client_session> client_sessions;
mutex sessions_mutex;

void broadcast_to_all(int sender_fd, const string& sender_name, const string& message) {
    lock_guard<mutex> lock(clients_mutex);
    for (int client_fd : active_clients) {
        if (client_fd != sender_fd) {
            send_packet(client_fd, 2, message);
        }
    }
}
...
void worker_thread_loop(int thread_id) {
    ...
    while (server_running) {
        code_task task;
        {
            ...
            pool_cv.wait(lock, [] { return !task_queue.empty() || !server_running; });
...
public:
    ...
    void server_main() {
        ...
        for (int i = 0; i < THREAD_POOL_SIZE; ++i) {
            thread_pool.emplace_back(worker_thread_loop, i);
        }
